import os
import interactions
from interactions import Client, Intents, listen, slash_command, SlashContext, OptionType, slash_option, Modal, ParagraphText, ShortText, User, Member, component_callback, modal_callback, ComponentContext, ComponentCommand, Button, ButtonStyle
from dotenv import load_dotenv

bot = Client(intents=Intents.GUILDS | Intents.GUILD_MEMBERS | Intents.GUILD_MESSAGES)  # Create a bot instance with the specified intents
...
async def on_button_click(ctx: ComponentContext):
    join_modal = Modal(
        ShortText(label="Name", required=True, placeholder="Enter your name", custom_id="name_modal"),
        ShortText(label="Email", required=True, placeholder="Enter your email", custom_id="email_modal"),
        ShortText(label="Student ID", required=True, placeholder="Enter your student ID", custom_id="student_id_modal"),
        title="Join Form",
        custom_id="join_form"
    )
...
## display modal when user joins the server
# @listen()
# async def on_member_join(event: interactions.api.events.discord.MemberAdd):
...
#     welcome_channel = await interactions.Channel.fetch("WELCOME_ID")
#     await welcome_channel.send(
#         f"Welcome {event.member.mention}! Please fill out the join form.",
#         components=button
#     )


# @component_callback("open_join_form")
# async def on_button_click(ctx: interactions.ComponentContext):
#     join_modal = interactions.Modal(
#         ShortText(label="Name", required=True, placeholder="Enter your name", custom_id="name_modal"),
#         ShortText(label="Email", required=True, placeholder="Enter your email", custom_id="email_modal"),
#         ShortText(label="Student ID", required=True, placeholder="Enter your student ID", custom_id="student_id_modal"),
#         title="Join Form",
#         custom_id="join_form"
#     )
#     await ctx.popup(join_modal)

# @modal_callback("join_form")
# async def on_modal_submit(ctx: interactions.ComponentContext, name_modal: str, email_modal: str, student_id_modal: str):
#     await ctx.send(
#         f"Thanks for submitting!\n**Name**: {name_modal}\n**Email**: {email_modal}\n**Student ID**: {student_id_modal}",
#         ephemeral=True
#     )
...
async def create_thread_with_option(ctx: SlashContext, string_option: str):
    thread = await ctx.channel.create_thread(name=string_option, auto_archive_duration=60)
